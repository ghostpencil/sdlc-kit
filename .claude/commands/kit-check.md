...
- **4 — `{{` census:** `grep -rc '{{' sdlc-kit/commands sdlc-kit/skills
  sdlc-kit/reference/REVIEW_LENSES.md` → hits in `sdlc-setup.md` only, and setup's
  close-out check names exactly `CLAUDE.md spec/ .claude/settings.json`.
...
- **1 — no project facts:** every declarative sentence in `commands/*.md` that could be
  false for *some* adopting project (a baseline state, a stack, a number, a tool) is a
  finding. Commands may only point at where the project records the fact.
...
- **5 — pointers resolve post-setup:** enumerate file-and-section pointers in installed
  files (`commands/`, `skills/`, `reference/REVIEW_LENSES.md`); verify each target
  exists in what setup installs or instantiates, not merely in this repo.
